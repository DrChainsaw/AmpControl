...
public class ResNetConv2DFactory {
    ...
    public static void main(String[] args) {
        ...
        final ISchedule lrSched = new Mul(new SawTooth(schedPeriod, 1e-6, 1e-2), new Step(schedPeriod, new Exponential(0.2)));
        final ISchedule momSched = new Offset(schedPeriod / 2, new SawTooth(schedPeriod, 0.85, 0.95));

        Plot.plot(IntStream.range(0, 2 * schedPeriod).mapToDouble(e -> lrSched.valueAt(0, e)).boxed().collect(Collectors.toList()), "Lr sched");
